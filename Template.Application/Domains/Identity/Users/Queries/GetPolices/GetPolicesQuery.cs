...
namespace Template.Application.Domains.Identity.Users.Queries.GetPolices;
...
{
    public GetPolicesQueryHandler(IContext context, HandlerDependencies<GetPolicesQuery, IEnumerable<KeyValuePairVM>> dependencies) : base(context, dependencies)
    {
    }
    ...
    protected override async Task<ApiResponse<IEnumerable<KeyValuePairVM>>> RunCore(GetPolicesQuery request, CancellationToken cancellationToken, object? additionalData)
        => new SucessoResponse<IEnumerable<KeyValuePairVM>>(
