...
namespace Template.Application.Domains.Identity.Users.Queries.GetAll;
...
public class GetAllQueryHandler : HandlerBase<GetAllQuery, IEnumerable<UserVm>>
{
    ...
    public GetAllQueryHandler(IContext context, HandlerDependencies<GetAllQuery, IEnumerable<UserVm>> dependencies) : base(context, dependencies)
    {
    ...
    protected override async Task<ApiResponse<IEnumerable<UserVm>>> RunCore(GetAllQuery request, CancellationToken cancellationToken, object? additionalData)
    {
        ...
        if (list.Dados == null || !list.Dados.Any())
            return new SucessoResponse<IEnumerable<UserVm>>("Consulta concluída com sucesso, porém nenhum registro foi encontrado.", null);

        foreach (var user in list.Dados)
        {
