...
namespace Template.Application.Domains.InternalServices.Clients.Queries.GetAll;
...
public class GetAllQueryHandler : HandlerBase<GetAllQuery, IEnumerable<ClientVM>>
{
    ...
    public GetAllQueryHandler(IContext context, HandlerDependencies<GetAllQuery, IEnumerable<ClientVM>> dependencies, IClientRepository repository) : base(context, dependencies)
    {
    ...
    protected override async Task<ApiResponse<IEnumerable<ClientVM>>> RunCore(GetAllQuery request, CancellationToken cancellationToken, object? additionalData = null)
    {
        ...
        var newList = list.Dados?.Select(x => new ClientVM(x.Id, x.FullName, x.DocumentNumber, x.Phone, x.ZipCode)).ToList();
