...
//TODO:Exemplo de como usar!!! 

/*

 var additionalParams = new { Param1 = "Value1", Param2 = 123 };
var response = await _handler.Executar(request, cancellationToken, additionalParams);
 

protected override async Task<ApiResponse<TResponse>> ExecutarCore(TRequest request, CancellationToken cancellationToken, object? additionalData)
{
    if (additionalData is not null)
    {
        // Verifique o tipo e extraia os valores
        if (additionalData is { Param1: string param1Value, Param2: int param2Value })
        {
            // Use `param1Value` e `param2Value` como necessário
        }
    }

    // Sua lógica principal do handler
}
 */